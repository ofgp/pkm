...
import static com.rst.pkm.common.Constant.CURVE;
...
public class ECDSASignature {
    ...
    public ECDSASignature(BigInteger r, BigInteger s, int recId) {
        this.r = r;
        this.s = s;
        this.recId = recId;
    }
    ...
    public byte[] compactForEOS(boolean isCompressed) {
        int curveLen = (CURVE.getCurve().getFieldSize() + 7) / 8;
        ...
        byte[] rBytes = integerToBytes(r, curveLen);
        byte[] sBytes = integerToBytes(s, curveLen);
    ...
    public ECDSASignature decodeFromEOSCompact(byte[] bytes) {
        int curveLen = (CURVE.getCurve().getFieldSize() + 7) / 8;
        int firstByte = (int)bytes[0];
        ...
        byte[] rBytes = new byte[curveLen];
        System.arraycopy(bytes, 1, rBytes, 0, curveLen);
        BigInteger r = new BigInteger(rBytes);
        byte[] sBytes = new byte[curveLen];
        System.arraycopy(bytes, 1 + curveLen, sBytes, 0, curveLen);
    ...
    public static byte[] recoverPubKey(byte[] messageSigned, ECDSASignature signature, int recId) {
        ...
        ECPoint q = sumOfTwoMultiplies( CURVE.getG(), eInvrInv, R, srInv); //  Secp256k1Param.G, eInvrInv, R, srInv);

        return q.getEncoded(true);
    }

    public static ECPoint decompressKey(BigInteger x, boolean firstBit) {
        int size = 1 + getByteLength(CURVE.getCurve().getFieldSize());
        byte[] dest = integerToBytes(x, size);
        dest[0] = (byte) (firstBit ? 0x03 : 0x02);
        return CURVE.getCurve().decodePoint(dest);
    }
